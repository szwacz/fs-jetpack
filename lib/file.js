...
var sync = function (path, criteria) {
    ...
    if (stat) {
        ...
        if (criteria.empty === false && criteria.content !== undefined) {
            fileUtil.write(path, criteria.content, { mode: mode });
        }
        
    } else if (criteria.exists === true) {
        // Create new file.
        fileUtil.write(path, criteria.content || '', { mode: criteria.mode });
    }
};
    ...
    var ensureCriteria = function (stat) {
        ...
        if (stat) {
            ...
            var ensureContent = function () {
                ...
                if (criteria.empty === false && criteria.content !== undefined) {
                    fileUtil.writeAsync(path, criteria.content, { mode: mode })
            ...
            fileUtil.writeAsync(path, criteria.content || '', { mode: criteria.mode })
            .then(deferred.resolve, deferred.reject);
        } else {
