    ...
    try {
        ...
        if (err.code === 'ENOENT') {
            try {
                ...
                throw err; // nope BAK path not present either, rethrow original error
            }
        } else {
...
function writeSafeSync(path, data) {
    ...
    fs.writeFileSync(path + newExt, data);
...
function remove(path, options) {
    ...
    if (options.safe === true) {
        try {
            ...
            // nothing there, but we had to try anyway for one of a million
            // chance for a bug if this file will remain there
        }
    }
}
    ...
    .then(deferred.resolve, function (err) {
        if (err.code === 'ENOENT') {
            ...
            .then(deferred.resolve, function (err2) {
                // nothing, rethrow original error
                deferred.reject(err);
            });
        } else {
            deferred.reject(err);
...
function writeSafeAsync(path, data) {
    var deferred = Q.defer();
    var bakFileSet = false;
...
function writeAsync(path, data, options) {
    ...
    if (options.safe === true) {
        writeSafeAsync(path, data)
        .then(deferred.resolve, deferred.reject);
    } else {
        qWriteFile(path, data)
        .then(deferred.resolve, deferred.reject);
    }
...
function removeAsync(path, options) {
    options = options || {};
    ...
    qUnlink(path)
    .then(function () {
        return qUnlink(path + bakExt);
    })
    .then(deferred.resolve, deferred.resolve) // we don't care if bak file exists so resolve anyway
...
module.exports.removeAsync = removeAsync;
