...
const parseOptions = (options, from) => {
  ...
  if (opts.matching) {
    ...
    parsedOptions.allowedToCopy = (fromPath, inspectData, toPath) => {
      // When a function is used, use it instead of default.
      if (typeof parsedOptions.overwrite === 'function') {
        return parsedOptions.overwrite(fromPath, inspectData, toPath);
      }
...
const copySync = (from, to, options) => {
  ...
  // Allows disabling the copy without try catches
  ...
  treeWalker.sync(from, {
    ...
    if (opts.allowedToCopy(path, destPath, inspectData)) {
