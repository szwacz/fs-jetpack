...
const createDirSync = (path, mode, context) => {
  try {
    dir.mkdirpSync(path, mode);
  } catch (err) {
    if (err.code === 'EEXIST') {
...
const copyFileSync = (srcPath, destPath, mode, context) => {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      dir.mkdirpSync(pathUtil.dirname(destPath));
...
const copyItemSync = (srcPath, srcInspectData, destPath, opts) => {
  ...
  if (srcInspectData.type === 'dir') {
    createDirSync(destPath, mode, context);
...
const checkSourceExistsAsync = (srcPath) => {
  return exists.async(srcPath)
  .then((srcPathExists) => {
    if (!srcPathExists) {
      throw generateNoSourceError(srcPath);
    }
  });
};
...
const createDirAsync = (path, mode, context) => {
  return new Promise((resolve, reject) => {
    dir.mkdirpAsync(path, mode)
    .then(resolve)
    .catch((err) => {
      if (err.code === 'EEXIST') {
        ...
        .then((canOverwite) => {
          if (canOverwite) {
            dir.async(path, { mode }).then(resolve, reject);
          } else if (shouldThrowDestinationExistsError(context)) {
            reject(generateDestinationExistsError(path));
          } else {
            resolve();
          }
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    writeStream.on('error', (err) => {
      ...
      if (err.code === 'ENOENT') {
        ...
        dir.mkdirpAsync(pathUtil.dirname(destPath))
        .then(() => {
...
const copyItemAsync = (srcPath, srcInspectData, destPath, opts) => {
  ...
  const mode = fileMode.normalizeFileMode(srcInspectData.mode);
  if (srcInspectData.type === 'dir') {
    return createDirAsync(destPath, mode, context);
  } else if (srcInspectData.type === 'file') {
