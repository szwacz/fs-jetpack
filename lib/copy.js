...
function processOptions(options, basePath) {
    ...
    if (options.only) {
        options.only = matcher.create(options.only, pathUtil.dirname(basePath));
    }
    if (options.allBut) {
        options.allBut = matcher.create(options.allBut, pathUtil.dirname(basePath));
    }
    return options;
}
...
module.exports.sync = function (from, to, options) {
    ...
    function copy(fromPath, toPath) {
        ...
        if (fs.existsSync(toPath) && options.overwrite === false) {
