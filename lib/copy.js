...
var normalizeMatching = function (matching, fromPath) {
    if (typeof matching === 'string') {
        matching = [matching];
    }

    return matching.map(function (pattern) {
        if (/^\.\//.test(pattern)) {
...
var sync = function (from, to, options) {
    ...
    if (options.matching) {
        allowedToCopy = matcher.create(normalizeMatching(options.matching, from));
    }
