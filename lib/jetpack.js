...
const jetpackContext = (cwdPath) => {
  ...
  const api = {
    ...
    realpath: (path) => {
      realpath.validateInput("realpath", path);
      return realpath.sync(resolvePath(path));
    },
    realpathAsync: (path) => {
      realpath.validateInput("realpathAsync", path);
      return realpath.async(resolvePath(path));
    },
