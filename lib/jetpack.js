...
var jetpackContext = function (cwdPath) {
  ...
  return {
    ...
    inspect: function (path, fieldsToInclude) {
      return inspector.inspectSync(resolvePath(path), fieldsToInclude);
    },
    ...
    inspectTree: function (path, options) {
      return inspector.inspectTreeSync(resolvePath(path), options);
    },
    inspectTreeAsync: function (path, options) {
      return inspector.inspectTreeAsync(resolvePath(path), options);
    },

    list: function (path) {
      return inspector.listSync(resolvePath(path || '.'));
    },
