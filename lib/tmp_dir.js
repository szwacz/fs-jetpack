...
const generateBasePathNotDirectoryError = path => {
  return new Error(`Passed option "basePath" is not a directory: ${path}.`);
};
...
const tmpDirSync = (cwdPath, passedOptions) => {
  ...
  try {
    ...
    if (err.code === "ENOENT") {
      ...
      throw generateBasePathNotDirectoryError(options.basePath);
    } else {
      throw err;
    }
  }
        ...
        fs.mkdir(dirPath, err => {
          if (err) {
            ...
            if (err.code === "ENOENT") {
              ...
              reject(generateBasePathNotDirectoryError(options.basePath));
            } else {
              reject(err);
            }
          } else {
