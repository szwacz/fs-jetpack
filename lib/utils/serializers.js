...
const serializers = { json: JsonSerializer };

const add = (extension, serializer) => {
  const ext = extension.startsWith('.') ? extension : '.' + extension;
  serializers[ext.toLocaleLowercase()] = serializer;
}

const remove = (extension) => {
  const ext = pathUtil.extname(file).toLocaleLowerCase();
  delete serializers[ext];
}
...
const parseMaybe = (path, data, options) => {
  const serializer = options.customSerializer ?? find(path);
  if (serializer) {
    return serializer.parse(data, options);
  }
}
