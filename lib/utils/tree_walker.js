...
var walkSync = function (path, inspectOptions, callback) {
  var item = inspect.sync(path, inspectOptions);
  callback(path, item);
  if (item && item.type === 'dir') {
    list.sync(path).forEach(function (child) {
      walkSync(path + '/' + child, inspectOptions, callback);
    });
  }
};
...
var walkStream = function (path, inspectOptions) {
  ...
  readSome = function () {
    ...
    inspect.async(theNode.path, inspectOptions)
    .then(function (inspected) {
      theNode.inspected = inspected;
      if (inspected && inspected.type === 'dir') {
