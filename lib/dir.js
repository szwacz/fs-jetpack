...
var sync = function (path, criteria) {
    ...
    if (stat) {
        ...
        if (!criteria.exists) {
            ...
            rimraf.sync(path);
            // No need to check more criteria, exists=false is ultimate.
            return;
        }
...
var async = function (path, criteria) {
    ...
    var checkWhatShouldBe = function (stat) {
        ...
        var checkExistence = function () {
            if (!stat && !criteria.exists) {
                // Path doesn't exist, and this is the way it should be.
                deferred.resolve();
            } else if (stat && !criteria.exists) {
                // Path exists, and we want for it not to exist
                promisedRimraf(path)
                .then(deferred.resolve, deferred.reject);
            } else if (!stat && criteria.exists) {
